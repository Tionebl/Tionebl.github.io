...
function addFilm(storageFilmName) {
  let data = [];
  const filmId = "filmsId";
  const select = document.getElementById("selectFilm");
  const genre = select.options[select.selectedIndex].value;
  const block = document.getElementById(filmId);
  ...
  const valueFilm = getRandomInt(2);
  if (genre == "Drame") {
    if (valueFilm == 0) {
      filmName = "Dallas buyers club";
      img.setAttribute("src", "./imgfilm/dbc.jpg");
    } else if (valueFilm == 1) {
      filmName = "Little Fish";
      img.setAttribute("src", "./imgfilm/littlefish.jpg");
    }
    ...
    if (valueFilm == 0) {
      ...
      img.setAttribute("src", "imgfilmmarrowbone.jpg");
    }
  ...
  // del.onclick="deleteChar(this)" ,id="binId";

  // del.setAttribute("onclick","document.getElementById("+indexFilm+").remove()");
}

function onLoadPage() {
  // recup donnée lors du chargement
...
//   console.log(e.parentElement.parentElement.filmId);
//   data.splice(e.parentElement.parentElement.filmId, 1);
